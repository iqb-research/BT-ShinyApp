...
if(any(params$data_selected$fach %in% c("Französisch", "French"))) {
  str_deutschland <- "Gesamt"
} else {
  str_deutschland <- "Deutschland"
}
str_bildungstrend <- "IQB Bildungstrend"
str_kompetentbereich <- "Kompetenzbereich"
str_source_intro <- "Die Daten sowie weitere Informationen sind dem entsprechenden Berichtsband zu entnehmen:"

if(params$language == "en"){
  str_bundesland <- recode(str_bundesland, !!! params$woerterbuch)
  str_deutschland <- recode(str_deutschland, !!! params$woerterbuch)
  str_bildungstrend <- recode(str_bildungstrend, !!! params$woerterbuch)
  str_kompetentbereich <- recode(str_kompetentbereich, !!! params$woerterbuch)
  str_source_intro <- recode(str_source_intro, !!! params$woerterbuch)
 }
```
...
untertitel <- paste0(
  str_kompetentbereich,
  " ",
  unique(na.omit(data$fachKb)), 
  ", ", 
  ifelse(params$language == "de", 
         gsub("alle", "alle Schüler:innen", unique(na.omit(data$targetPop))),
         gsub("All", "all students", unique(na.omit(data$targetPop))))
)
```
...
karte <- ggplot(data) +
  ...
  theme(
    text = element_text(family = "sans")  # Schriftart für Text
  )
...
if(params$language == "en"){
  table_data[,1] <- recode(table_data[,1], !!! params$woerterbuch)
}
...
tabelle <- gridExtra::tableGrob(
  ...
  theme = gridExtra::ttheme_default(
    core = list(
      fg_params = list(cex = 0.8, fontfamily = "sans")  # Schriftart für Tabellenzellen
    ),
    colhead = list(
      fg_params = list(cex = 0.9, fontfamily = "sans")  # Schriftart für Kopfzeilen
    )
  )
)
